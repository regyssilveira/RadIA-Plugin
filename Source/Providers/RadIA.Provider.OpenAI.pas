...
type
  ...
  private
    ...
    function NormalizeCodexResponseError(const AResponse: string): string;
...
uses
  System.Classes, System.StrUtils, Winapi.Windows,
...
function TRadIAOpenAIProvider.NormalizeCodexResponseError(
  const AResponse: string
): string;
begin
  Result := AResponse.Trim;
  if ContainsText(Result, 'requires a newer version of Codex') then
    Exit(
      'The selected model requires a newer Codex CLI. RadIA native is orchestrating this request, ' +
      'but ChatGPT Pro still uses Codex CLI as its transport. Open Settings > CLI & MCP, select ' +
      'Codex CLI, click Update channel or choose a newer executable, run Diagnose, refresh the ' +
      'model list, and retry.'
    );
  if Result.StartsWith('Error: No response generated by Codex.', True) then
    Exit(
      'Codex CLI returned no assistant response. Refresh the model list and select a supported ' +
      'model, then retry. If the problem continues, run /doctor --deep.'
    );
  if Result.StartsWith('Error:', True) then
    Result := 'Codex CLI error:' + Result.Substring(Length('Error:'));
end;
...
begin
  ...
  if Result.IsEmpty then
    Result := AFallback;
  Result := NormalizeCodexResponseError('Codex CLI error: ' + Result.Trim);
end;
