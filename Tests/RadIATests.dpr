program RadIATests;

{$DEFINE TESTS}
{$APPTYPE CONSOLE}

uses
  System.IOUtils,
  System.SysUtils,
  DUnitX.Loggers.Console,
  DUnitX.Loggers.XML.NUnit,
  DUnitX.TestFramework,

  // Production Units
  RadIA.Core.Types in '..\Source\Core\RadIA.Core.Types.pas',
  RadIA.Core.DelphiSignatures in '..\Source\Core\RadIA.Core.DelphiSignatures.pas',
  RadIA.Tests.DelphiSignatures in 'Source\RadIA.Tests.DelphiSignatures.pas',
  RadIA.Core.DelphiCallArguments in '..\Source\Core\RadIA.Core.DelphiCallArguments.pas',
  RadIA.Core.DelphiExtraction in '..\Source\Core\RadIA.Core.DelphiExtraction.pas',
  RadIA.Tests.DelphiExtraction in 'Source\RadIA.Tests.DelphiExtraction.pas',
  RadIA.Core.DelphiTypeMove in '..\Source\Core\RadIA.Core.DelphiTypeMove.pas',
  RadIA.Core.CodeValidation in '..\Source\Core\RadIA.Core.CodeValidation.pas',
  RadIA.Core.CodeValidationFixes in '..\Source\Core\RadIA.Core.CodeValidationFixes.pas',
  RadIA.Core.DelphiLintAdapter in '..\Source\Core\RadIA.Core.DelphiLintAdapter.pas',
  RadIA.Core.CodeValidationTools in '..\Source\Core\RadIA.Core.CodeValidationTools.pas',
  RadIA.Tests.DelphiTypeMove in 'Source\RadIA.Tests.DelphiTypeMove.pas',
  RadIA.Tests.CodeValidation in 'Source\RadIA.Tests.CodeValidation.pas',
  RadIA.Core.SemanticExtractMethodTools in '..\Source\Core\RadIA.Core.SemanticExtractMethodTools.pas',
  RadIA.Core.SemanticChangeSignatureTools in '..\Source\Core\RadIA.Core.SemanticChangeSignatureTools.pas',
  RadIA.Core.SemanticMoveTypeTools in '..\Source\Core\RadIA.Core.SemanticMoveTypeTools.pas',
  RadIA.Tests.DelphiCallArguments in 'Source\RadIA.Tests.DelphiCallArguments.pas',
  RadIA.Core.Version in '..\Source\Core\RadIA.Core.Version.pas',
  RadIA.Semantic.Lexer in '..\Source\Semantic\RadIA.Semantic.Lexer.pas',
  RadIA.Tests.SemanticLexer in 'Source\RadIA.Tests.SemanticLexer.pas',
  RadIA.Semantic.Preprocessor in '..\Source\Semantic\RadIA.Semantic.Preprocessor.pas',
  RadIA.Tests.SemanticPreprocessor in 'Source\RadIA.Tests.SemanticPreprocessor.pas',
  RadIA.Semantic.Parser in '..\Source\Semantic\RadIA.Semantic.Parser.pas',
  RadIA.Tests.SemanticParser in 'Source\RadIA.Tests.SemanticParser.pas',
  RadIA.Semantic.Index in '..\Source\Semantic\RadIA.Semantic.Index.pas',
  RadIA.Tests.SemanticIndex in 'Source\RadIA.Tests.SemanticIndex.pas',
  RadIA.Semantic.MissingMembers in '..\Source\Semantic\RadIA.Semantic.MissingMembers.pas',
  RadIA.Tests.SemanticMissingMembers in 'Source\RadIA.Tests.SemanticMissingMembers.pas',
  RadIA.Semantic.Client in '..\Source\Semantic\RadIA.Semantic.Client.pas',
  RadIA.Tests.SemanticClient in 'Source\RadIA.Tests.SemanticClient.pas',
  RadIA.Semantic.Workspace in '..\Source\Semantic\RadIA.Semantic.Workspace.pas',
  RadIA.Tests.SemanticWorkspace in 'Source\RadIA.Tests.SemanticWorkspace.pas',
  RadIA.Core.SemanticMembers in '..\Source\Core\RadIA.Core.SemanticMembers.pas',
  RadIA.Core.SemanticMemberTools in '..\Source\Core\RadIA.Core.SemanticMemberTools.pas',
  RadIA.Tests.SemanticMembers in 'Source\RadIA.Tests.SemanticMembers.pas',
  RadIA.Core.SemanticQueries in '..\Source\Core\RadIA.Core.SemanticQueries.pas',
  RadIA.Core.SemanticQueryTools in '..\Source\Core\RadIA.Core.SemanticQueryTools.pas',
  RadIA.Core.SemanticHierarchyTools in '..\Source\Core\RadIA.Core.SemanticHierarchyTools.pas',
  RadIA.Core.SemanticRefactoringTools in '..\Source\Core\RadIA.Core.SemanticRefactoringTools.pas',
  RadIA.Core.TransactionalTextFiles in '..\Source\Core\RadIA.Core.TransactionalTextFiles.pas',
  RadIA.Core.TestImpact in '..\Source\Core\RadIA.Core.TestImpact.pas',
  RadIA.Core.TestImpactTools in '..\Source\Core\RadIA.Core.TestImpactTools.pas',
  RadIA.Tests.TestImpact in 'Source\RadIA.Tests.TestImpact.pas',
  RadIA.Tests.SemanticQueries in 'Source\RadIA.Tests.SemanticQueries.pas',
  RadIA.Tests.SemanticRefactoring in 'Source\RadIA.Tests.SemanticRefactoring.pas',
  RadIA.Core.SemanticCompletion in '..\Source\Core\RadIA.Core.SemanticCompletion.pas',
  RadIA.Tests.SemanticCompletion in 'Source\RadIA.Tests.SemanticCompletion.pas',
  RadIA.Core.Tools in '..\Source\Core\RadIA.Core.Tools.pas',
  RadIA.Core.AgentRuntime in '..\Source\Core\RadIA.Core.AgentRuntime.pas',
  RadIA.Core.ResultCompactor in '..\Source\Core\RadIA.Core.ResultCompactor.pas',
  RadIA.Core.ResultCompactionSettings in '..\Source\Core\RadIA.Core.ResultCompactionSettings.pas',
  RadIA.Core.AgentResultStore in '..\Source\Core\RadIA.Core.AgentResultStore.pas',
  RadIA.Core.AgentResultTools in '..\Source\Core\RadIA.Core.AgentResultTools.pas',
  RadIA.Core.AgentDiagnostic in '..\Source\Core\RadIA.Core.AgentDiagnostic.pas',
  RadIA.Core.AgentExecutors in '..\Source\Core\RadIA.Core.AgentExecutors.pas',
  RadIA.Core.AgentExecutorContracts in '..\Source\Core\RadIA.Core.AgentExecutorContracts.pas',
  RadIA.Core.JourneyContext in '..\Source\Core\RadIA.Core.JourneyContext.pas',
  RadIA.Core.HierarchicalSettings in '..\Source\Core\RadIA.Core.HierarchicalSettings.pas',
  RadIA.Core.HierarchicalSettingsStore in '..\Source\Core\RadIA.Core.HierarchicalSettingsStore.pas',
  RadIA.Core.CliProcess in '..\Source\Core\RadIA.Core.CliProcess.pas',
  RadIA.Core.PseudoTerminal in '..\Source\Core\RadIA.Core.PseudoTerminal.pas',
  RadIA.Core.Terminal in '..\Source\Core\RadIA.Core.Terminal.pas',
  RadIA.Core.TerminalDiagnostics in '..\Source\Core\RadIA.Core.TerminalDiagnostics.pas',
  RadIA.Core.WebViewLifecycle in '..\Source\Core\RadIA.Core.WebViewLifecycle.pas',
  RadIA.Core.TerminalScreen in '..\Source\Core\RadIA.Core.TerminalScreen.pas',
  RadIA.Core.TerminalEmulator in '..\Source\Core\RadIA.Core.TerminalEmulator.pas',
  RadIA.UI.TerminalFrame in '..\Source\UI\RadIA.UI.TerminalFrame.pas',
  RadIA.Core.Onboarding in '..\Source\Core\RadIA.Core.Onboarding.pas',
  RadIA.UI.OnboardingForm in '..\Source\UI\RadIA.UI.OnboardingForm.pas',
  RadIA.UI.Resources in '..\Source\UI\RadIA.UI.Resources.pas',
  RadIA.Tests.OnboardingForm in 'Source\RadIA.Tests.OnboardingForm.pas',
  RadIA.Core.AgentProvider in '..\Source\Core\RadIA.Core.AgentProvider.pas',
  RadIA.Core.AgentController in '..\Source\Core\RadIA.Core.AgentController.pas',
  RadIA.Core.AgentPricing in '..\Source\Core\RadIA.Core.AgentPricing.pas',
  RadIA.Core.CliManager in '..\Source\Core\RadIA.Core.CliManager.pas',
  RadIA.Core.CliMcpSettings in '..\Source\Core\RadIA.Core.CliMcpSettings.pas',
  RadIA.Core.McpProvisioning in '..\Source\Core\RadIA.Core.McpProvisioning.pas',
  RadIA.Core.McpHandshake in '..\Source\Core\RadIA.Core.McpHandshake.pas',
  RadIA.Core.ExternalMcp in '..\Source\Core\RadIA.Core.ExternalMcp.pas',
  RadIA.Core.ExternalMcpContent in '..\Source\Core\RadIA.Core.ExternalMcpContent.pas',
  RadIA.Core.ExternalMcpTransport in '..\Source\Core\RadIA.Core.ExternalMcpTransport.pas',
  RadIA.Core.ExternalMcpClient in '..\Source\Core\RadIA.Core.ExternalMcpClient.pas',
  RadIA.Core.ExternalMcpSecurity in '..\Source\Core\RadIA.Core.ExternalMcpSecurity.pas',
  RadIA.Core.ExternalMcpSettings in '..\Source\Core\RadIA.Core.ExternalMcpSettings.pas',
  RadIA.Core.ExternalMcpRuntime in '..\Source\Core\RadIA.Core.ExternalMcpRuntime.pas',
  RadIA.Core.ExternalMcpImport in '..\Source\Core\RadIA.Core.ExternalMcpImport.pas',
  RadIA.Tests.ExternalMcpRuntime in 'Source\RadIA.Tests.ExternalMcpRuntime.pas',
  RadIA.Tests.ExternalMcpImport in 'Source\RadIA.Tests.ExternalMcpImport.pas',
  RadIA.Core.ToolRegistry in '..\Source\Core\RadIA.Core.ToolRegistry.pas',
  RadIA.Core.ToolViews in '..\Source\Core\RadIA.Core.ToolViews.pas',
  RadIA.Core.Problems in '..\Source\Core\RadIA.Core.Problems.pas',
  RadIA.Core.Extensions in '..\Source\Core\RadIA.Core.Extensions.pas',
  RadIA.Core.DeclarativeExtensions in '..\Source\Core\RadIA.Core.DeclarativeExtensions.pas',
  RadIA.Core.DeclarativeTools in '..\Source\Core\RadIA.Core.DeclarativeTools.pas',
  RadIA.Core.DeclarativeExtensionPackages in '..\Source\Core\RadIA.Core.DeclarativeExtensionPackages.pas',
  RadIA.Core.RsaSignature in '..\Source\Core\RadIA.Core.RsaSignature.pas',
  RadIA.Core.ExtensionPublisherTrust in '..\Source\Core\RadIA.Core.ExtensionPublisherTrust.pas',
  RadIA.Core.ExtensionCatalog in '..\Source\Core\RadIA.Core.ExtensionCatalog.pas',
  RadIA.Core.ExtensionStudio in '..\Source\Core\RadIA.Core.ExtensionStudio.pas',
  RadIA.Core.SkillPortability in '..\Source\Core\RadIA.Core.SkillPortability.pas',
  RadIA.Core.SkillReplicas in '..\Source\Core\RadIA.Core.SkillReplicas.pas',
  RadIA.UI.SkillPortabilityForm in '..\Source\UI\RadIA.UI.SkillPortabilityForm.pas',
  RadIA.Core.ExtensionSigning in '..\Source\Core\RadIA.Core.ExtensionSigning.pas',
  RadIA.UI.ExtensionCatalogForm in '..\Source\UI\RadIA.UI.ExtensionCatalogForm.pas',
  RadIA.Core.ToolSecurity in '..\Source\Core\RadIA.Core.ToolSecurity.pas',
  RadIA.Core.ConsentPresentation in '..\Source\Core\RadIA.Core.ConsentPresentation.pas',
  RadIA.Core.ConsentGate in '..\Source\Core\RadIA.Core.ConsentGate.pas',
  RadIA.Core.Patches in '..\Source\Core\RadIA.Core.Patches.pas',
  RadIA.Core.BlockReviews in '..\Source\Core\RadIA.Core.BlockReviews.pas',
  RadIA.Core.BlockReviewSessions in '..\Source\Core\RadIA.Core.BlockReviewSessions.pas',
  RadIA.Core.BlockReviewTools in '..\Source\Core\RadIA.Core.BlockReviewTools.pas',
  RadIA.Core.PatchTools in '..\Source\Core\RadIA.Core.PatchTools.pas',
  RadIA.Core.MultiFilePatches in '..\Source\Core\RadIA.Core.MultiFilePatches.pas',
  RadIA.Core.MultiFilePatchTools in '..\Source\Core\RadIA.Core.MultiFilePatchTools.pas',
  RadIA.Core.DevelopmentTransactions in '..\Source\Core\RadIA.Core.DevelopmentTransactions.pas',
  RadIA.Core.DevelopmentTransactionTools in '..\Source\Core\RadIA.Core.DevelopmentTransactionTools.pas',
  RadIA.Core.Build in '..\Source\Core\RadIA.Core.Build.pas',
  RadIA.Core.BuildTools in '..\Source\Core\RadIA.Core.BuildTools.pas',
  RadIA.Core.DUnitX in '..\Source\Core\RadIA.Core.DUnitX.pas',
  RadIA.Core.DUnitXTools in '..\Source\Core\RadIA.Core.DUnitXTools.pas',
  RadIA.Core.Coverage in '..\Source\Core\RadIA.Core.Coverage.pas',
  RadIA.Core.CoverageTools in '..\Source\Core\RadIA.Core.CoverageTools.pas',
  RadIA.Core.Git in '..\Source\Core\RadIA.Core.Git.pas',
  RadIA.Core.GitTools in '..\Source\Core\RadIA.Core.GitTools.pas',
  RadIA.OTA.Git in '..\Source\Integration\RadIA.OTA.Git.pas',
  RadIA.Core.Mcp in '..\Source\Core\RadIA.Core.Mcp.pas',
  RadIA.Core.Designer in '..\Source\Core\RadIA.Core.Designer.pas',
  RadIA.Core.DesignerTools in '..\Source\Core\RadIA.Core.DesignerTools.pas',
  RadIA.Core.DesignerMutations in '..\Source\Core\RadIA.Core.DesignerMutations.pas',
  RadIA.Core.DesignerMutationTools in '..\Source\Core\RadIA.Core.DesignerMutationTools.pas',
  RadIA.Core.DesignerProperties in '..\Source\Core\RadIA.Core.DesignerProperties.pas',
  RadIA.Core.DesignerPropertyTools in '..\Source\Core\RadIA.Core.DesignerPropertyTools.pas',
  RadIA.Core.DesignerComponents in '..\Source\Core\RadIA.Core.DesignerComponents.pas',
  RadIA.Core.DesignerComponentTools in '..\Source\Core\RadIA.Core.DesignerComponentTools.pas',
  RadIA.Core.DesignerEvents in '..\Source\Core\RadIA.Core.DesignerEvents.pas',
  RadIA.Core.DesignerEventTools in '..\Source\Core\RadIA.Core.DesignerEventTools.pas',
  RadIA.Core.Debugger in '..\Source\Core\RadIA.Core.Debugger.pas',
  RadIA.Core.DebugTimeline in '..\Source\Core\RadIA.Core.DebugTimeline.pas',
  RadIA.Core.DebugTimelineTools in '..\Source\Core\RadIA.Core.DebugTimelineTools.pas',
  RadIA.Core.DebuggerTools in '..\Source\Core\RadIA.Core.DebuggerTools.pas',
  RadIA.Core.DebuggerControlTools in '..\Source\Core\RadIA.Core.DebuggerControlTools.pas',
  RadIA.Core.DebuggerBreakpointTools in '..\Source\Core\RadIA.Core.DebuggerBreakpointTools.pas',
  RadIA.Core.DebuggerWatches in '..\Source\Core\RadIA.Core.DebuggerWatches.pas',
  RadIA.Core.DebuggerInspectionTools in '..\Source\Core\RadIA.Core.DebuggerInspectionTools.pas',
  RadIA.Core.RuntimeAutomation in '..\Source\Core\RadIA.Core.RuntimeAutomation.pas',
  RadIA.Core.RuntimeVclAdapter in '..\Source\Core\RadIA.Core.RuntimeVclAdapter.pas',
  RadIA.Core.RuntimeVclInstrumentation in '..\Source\Core\RadIA.Core.RuntimeVclInstrumentation.pas',
  RadIA.Core.RuntimePerformance in '..\Source\Core\RadIA.Core.RuntimePerformance.pas',
  RadIA.Core.FireDAC.Model in '..\Source\Core\RadIA.Core.FireDAC.Model.pas',
  RadIA.Core.FireDAC.PascalMask in '..\Source\Core\RadIA.Core.FireDAC.PascalMask.pas',
  RadIA.Core.FireDAC.Configuration in '..\Source\Core\RadIA.Core.FireDAC.Configuration.pas',
  RadIA.Core.FireDAC.Environment in '..\Source\Core\RadIA.Core.FireDAC.Environment.pas',
  RadIA.Core.FireDAC.Scanner in '..\Source\Core\RadIA.Core.FireDAC.Scanner.pas',
  RadIA.Core.FireDAC.SqlAnalyzer in '..\Source\Core\RadIA.Core.FireDAC.SqlAnalyzer.pas',
  RadIA.Core.FireDAC.SqlExtraction in '..\Source\Core\RadIA.Core.FireDAC.SqlExtraction.pas',
  RadIA.Core.FireDAC.Schema in '..\Source\Core\RadIA.Core.FireDAC.Schema.pas',
  RadIA.Core.FireDAC.Parameters in '..\Source\Core\RadIA.Core.FireDAC.Parameters.pas',
  RadIA.Core.FireDAC.Transactions in '..\Source\Core\RadIA.Core.FireDAC.Transactions.pas',
  RadIA.Core.FireDAC.ThreadSafety in '..\Source\Core\RadIA.Core.FireDAC.ThreadSafety.pas',
  RadIA.Core.FireDAC.Tools in '..\Source\Core\RadIA.Core.FireDAC.Tools.pas',
  RadIA.Core.FireDAC.Generation in '..\Source\Core\RadIA.Core.FireDAC.Generation.pas',
  RadIA.Core.FireDAC.Fixes in '..\Source\Core\RadIA.Core.FireDAC.Fixes.pas',
  RadIA.Core.FireDAC.Plans in '..\Source\Core\RadIA.Core.FireDAC.Plans.pas',
  RadIA.Core.DelphiEcosystemTools in '..\Source\Core\RadIA.Core.DelphiEcosystemTools.pas',
  RadIA.Core.LocalDatabase in '..\Source\Core\RadIA.Core.LocalDatabase.pas',
  RadIA.Core.LocalDatabaseTools in '..\Source\Core\RadIA.Core.LocalDatabaseTools.pas',
  RadIA.Tests.LocalDatabase in 'Source\RadIA.Tests.LocalDatabase.pas',
  RadIA.Tests.RuntimePerformance in 'Source\RadIA.Tests.RuntimePerformance.pas',
  RadIA.Tests.FireDACModel in 'Source\RadIA.Tests.FireDACModel.pas',
  RadIA.Tests.FireDACConfiguration in 'Source\RadIA.Tests.FireDACConfiguration.pas',
  RadIA.Tests.FireDACEnvironment in 'Source\RadIA.Tests.FireDACEnvironment.pas',
  RadIA.Tests.FireDACScanner in 'Source\RadIA.Tests.FireDACScanner.pas',
  RadIA.Tests.FireDACSqlAnalyzer in 'Source\RadIA.Tests.FireDACSqlAnalyzer.pas',
  RadIA.Tests.FireDACSqlExtraction in 'Source\RadIA.Tests.FireDACSqlExtraction.pas',
  RadIA.Tests.FireDACSchema in 'Source\RadIA.Tests.FireDACSchema.pas',
  RadIA.Tests.FireDACParameters in 'Source\RadIA.Tests.FireDACParameters.pas',
  RadIA.Tests.FireDACTransactions in 'Source\RadIA.Tests.FireDACTransactions.pas',
  RadIA.Tests.FireDACThreadSafety in 'Source\RadIA.Tests.FireDACThreadSafety.pas',
  RadIA.Tests.FireDACTools in 'Source\RadIA.Tests.FireDACTools.pas',
  RadIA.Tests.FireDACGeneration in 'Source\RadIA.Tests.FireDACGeneration.pas',
  RadIA.Tests.FireDACFixes in 'Source\RadIA.Tests.FireDACFixes.pas',
  RadIA.Tests.FireDACPlans in 'Source\RadIA.Tests.FireDACPlans.pas',
  RadIA.Tests.DelphiEcosystemTools in 'Source\RadIA.Tests.DelphiEcosystemTools.pas',
  RadIA.OTA.RuntimePerformance in '..\Source\Integration\RadIA.OTA.RuntimePerformance.pas',
  RadIA.OTA.RuntimeVclTransport in '..\Source\Integration\RadIA.OTA.RuntimeVclTransport.pas',
  RadIA.Runtime.VclAdapter in '..\Source\Runtime\RadIA.Runtime.VclAdapter.pas',
  RadIA.Runtime.VclServer in '..\Source\Runtime\RadIA.Runtime.VclServer.pas',
  RadIA.Core.RuntimeScenario in '..\Source\Core\RadIA.Core.RuntimeScenario.pas',
  RadIA.Core.RuntimeScenarioTools in '..\Source\Core\RadIA.Core.RuntimeScenarioTools.pas',
  RadIA.Core.RuntimeEvidence in '..\Source\Core\RadIA.Core.RuntimeEvidence.pas',
  RadIA.Core.RuntimeEvidenceTools in '..\Source\Core\RadIA.Core.RuntimeEvidenceTools.pas',
  RadIA.Core.RuntimeRegression in '..\Source\Core\RadIA.Core.RuntimeRegression.pas',
  RadIA.Core.RuntimeRegressionTools in '..\Source\Core\RadIA.Core.RuntimeRegressionTools.pas',
  RadIA.Core.RuntimeDebugSession in '..\Source\Core\RadIA.Core.RuntimeDebugSession.pas',
  RadIA.Core.VisualRuntimeSession in '..\Source\Core\RadIA.Core.VisualRuntimeSession.pas',
  RadIA.Core.RuntimeVisualTools in '..\Source\Core\RadIA.Core.RuntimeVisualTools.pas',
  RadIA.Core.RuntimeDebugTools in '..\Source\Core\RadIA.Core.RuntimeDebugTools.pas',
  RadIA.Core.RuntimeDiscoveryTools in '..\Source\Core\RadIA.Core.RuntimeDiscoveryTools.pas',
  RadIA.Core.MemoryDiagnostics in '..\Source\Core\RadIA.Core.MemoryDiagnostics.pas',
  RadIA.Core.FastMM5 in '..\Source\Core\RadIA.Core.FastMM5.pas',
  RadIA.Core.MemoryInstrumentation in '..\Source\Core\RadIA.Core.MemoryInstrumentation.pas',
  RadIA.Core.FastMM5LogParser in '..\Source\Core\RadIA.Core.FastMM5LogParser.pas',
  RadIA.Core.MemoryDiagnosticSession in '..\Source\Core\RadIA.Core.MemoryDiagnosticSession.pas',
  RadIA.Core.MemoryEvidence in '..\Source\Core\RadIA.Core.MemoryEvidence.pas',
  RadIA.OTA.MemoryDiagnostic in '..\Source\Integration\RadIA.OTA.MemoryDiagnostic.pas',
  RadIA.Core.InlineReviews in '..\Source\Core\RadIA.Core.InlineReviews.pas',
  RadIA.Core.InlineCompletion in '..\Source\Core\RadIA.Core.InlineCompletion.pas',
  RadIA.Core.EditorContext in '..\Source\Core\RadIA.Core.EditorContext.pas',
  RadIA.Core.InlineShortcuts in '..\Source\Core\RadIA.Core.InlineShortcuts.pas',
  RadIA.Core.InlineReviewTools in '..\Source\Core\RadIA.Core.InlineReviewTools.pas',
  RadIA.Core.IDENavigation in '..\Source\Core\RadIA.Core.IDENavigation.pas',
  RadIA.Core.IDENavigationTools in '..\Source\Core\RadIA.Core.IDENavigationTools.pas',
  RadIA.Core.Knowledge in '..\Source\Core\RadIA.Core.Knowledge.pas',
  RadIA.Core.KnowledgeEmbeddings in '..\Source\Core\RadIA.Core.KnowledgeEmbeddings.pas',
  RadIA.Core.RemoteKnowledgeEmbeddings in '..\Source\Core\RadIA.Core.RemoteKnowledgeEmbeddings.pas',
  RadIA.Core.KnowledgeEmbeddingSelection in '..\Source\Core\RadIA.Core.KnowledgeEmbeddingSelection.pas',
  RadIA.Core.RemoteKnowledgeSettings in '..\Source\Core\RadIA.Core.RemoteKnowledgeSettings.pas',
  RadIA.Core.KnowledgePrivacy in '..\Source\Core\RadIA.Core.KnowledgePrivacy.pas',
  RadIA.Core.KnowledgeHistory in '..\Source\Core\RadIA.Core.KnowledgeHistory.pas',
  RadIA.Core.ProjectHealthTools in '..\Source\Core\RadIA.Core.ProjectHealthTools.pas',
  RadIA.Core.InstallationHealthTools in '..\Source\Core\RadIA.Core.InstallationHealthTools.pas',
  RadIA.Core.Journeys in '..\Source\Core\RadIA.Core.Journeys.pas',
  RadIA.Core.IntentRouter in '..\Source\Core\RadIA.Core.IntentRouter.pas',
  RadIA.Core.IntentTelemetry in '..\Source\Core\RadIA.Core.IntentTelemetry.pas',
  RadIA.Core.KnowledgeTools in '..\Source\Core\RadIA.Core.KnowledgeTools.pas',
  RadIA.Core.KnowledgeStore in '..\Source\Core\RadIA.Core.KnowledgeStore.pas',
  RadIA.Core.KnowledgeScheduler in '..\Source\Core\RadIA.Core.KnowledgeScheduler.pas',
  RadIA.Core.Workspace in '..\Source\Core\RadIA.Core.Workspace.pas',
  RadIA.Core.WorkspaceBoundary in '..\Source\Core\RadIA.Core.WorkspaceBoundary.pas',
  RadIA.Core.WorkspaceTools in '..\Source\Core\RadIA.Core.WorkspaceTools.pas',
  RadIA.Core.DelphiEnvironment in '..\Source\Core\RadIA.Core.DelphiEnvironment.pas',
  RadIA.Core.DelphiEnvironmentTools in '..\Source\Core\RadIA.Core.DelphiEnvironmentTools.pas',
  RadIA.Core.DelphiGuidance in '..\Source\Core\RadIA.Core.DelphiGuidance.pas',
  RadIA.Core.DelphiGuidanceTools in '..\Source\Core\RadIA.Core.DelphiGuidanceTools.pas',
  RadIA.Core.DelphiMentor in '..\Source\Core\RadIA.Core.DelphiMentor.pas',
  RadIA.Core.DfmPasAudit in '..\Source\Core\RadIA.Core.DfmPasAudit.pas',
  RadIA.Core.DfmPasAuditTools in '..\Source\Core\RadIA.Core.DfmPasAuditTools.pas',
  RadIA.Core.DesignerVisualDiff in '..\Source\Core\RadIA.Core.DesignerVisualDiff.pas',
  RadIA.Core.DesignerVisualDiffTools in '..\Source\Core\RadIA.Core.DesignerVisualDiffTools.pas',
  RadIA.Core.LegacyDataMigration in '..\Source\Core\RadIA.Core.LegacyDataMigration.pas',
  RadIA.Core.LegacyDataMigrationTools in '..\Source\Core\RadIA.Core.LegacyDataMigrationTools.pas',
  RadIA.Core.Interfaces in '..\Source\Core\RadIA.Core.Interfaces.pas',
  RadIA.Core.ChatMessage in '..\Source\Core\RadIA.Core.ChatMessage.pas',
  RadIA.Core.Container in '..\Source\Core\RadIA.Core.Container.pas',
  RadIA.Core.Logger in '..\Source\Core\RadIA.Core.Logger.pas',
  RadIA.Core.ConfigDefaults in '..\Source\Core\RadIA.Core.ConfigDefaults.pas',
  RadIA.Core.CredentialProtector in '..\Source\Core\RadIA.Core.CredentialProtector.pas',
  RadIA.Core.Config in '..\Source\Core\RadIA.Core.Config.pas',
  RadIA.Core.SettingsStorage in '..\Source\Core\RadIA.Core.SettingsStorage.pas',
  RadIA.Core.ProviderRegistry in '..\Source\Core\RadIA.Core.ProviderRegistry.pas',
  RadIA.Core.Cache in '..\Source\Core\RadIA.Core.Cache.pas',
  RadIA.Core.Service in '..\Source\Core\RadIA.Core.Service.pas',
  RadIA.Core.PromptHistory in '..\Source\Core\RadIA.Core.PromptHistory.pas',
  RadIA.Core.TokenUsage in '..\Source\Core\RadIA.Core.TokenUsage.pas',
  RadIA.Core.ConversationExporter in '..\Source\Core\RadIA.Core.ConversationExporter.pas',
  RadIA.Core.PromptTemplates in '..\Source\Core\RadIA.Core.PromptTemplates.pas',
  RadIA.Core.ProjectContext in '..\Source\Core\RadIA.Core.ProjectContext.pas',
  RadIA.Core.Sessions in '..\Source\Core\RadIA.Core.Sessions.pas',
  RadIA.Core.DTO.Generator in '..\Source\Core\RadIA.Core.DTO.Generator.pas',
  RadIA.Core.ProjectGenerator in '..\Source\Core\RadIA.Core.ProjectGenerator.pas',
  RadIA.Core.ProjectFiles in '..\Source\Core\RadIA.Core.ProjectFiles.pas',
  RadIA.Core.ProjectFileTools in '..\Source\Core\RadIA.Core.ProjectFileTools.pas',
  RadIA.Core.GeneratedArtifacts in '..\Source\Core\RadIA.Core.GeneratedArtifacts.pas',
  RadIA.Core.ProductivityGeneration in '..\Source\Core\RadIA.Core.ProductivityGeneration.pas',
  RadIA.Core.ProductivityGenerationTools in '..\Source\Core\RadIA.Core.ProductivityGenerationTools.pas',
  RadIA.Core.StackTraceAnalysis in '..\Source\Core\RadIA.Core.StackTraceAnalysis.pas',
  RadIA.Core.StackTraceTools in '..\Source\Core\RadIA.Core.StackTraceTools.pas',
  RadIA.Core.SaveReview in '..\Source\Core\RadIA.Core.SaveReview.pas',
  RadIA.Core.CleanUses in '..\Source\Core\RadIA.Core.CleanUses.pas',
  RadIA.Core.CleanUsesTools in '..\Source\Core\RadIA.Core.CleanUsesTools.pas',
  RadIA.Core.ThreadingAssistant in '..\Source\Core\RadIA.Core.ThreadingAssistant.pas',
  RadIA.Core.ThreadingAssistantTools in '..\Source\Core\RadIA.Core.ThreadingAssistantTools.pas',
  RadIA.Core.OpenApiRetrofit in '..\Source\Core\RadIA.Core.OpenApiRetrofit.pas',
  RadIA.Core.OpenApiRetrofitTools in '..\Source\Core\RadIA.Core.OpenApiRetrofitTools.pas',
  RadIA.Core.DextFormModernization in '..\Source\Core\RadIA.Core.DextFormModernization.pas',
  RadIA.Core.DextFormModernizationTools in '..\Source\Core\RadIA.Core.DextFormModernizationTools.pas',
  RadIA.Core.ProjectOpening in '..\Source\Core\RadIA.Core.ProjectOpening.pas',
  RadIA.Core.ApiSpecifications in '..\Source\Core\RadIA.Core.ApiSpecifications.pas',
  RadIA.Core.DextProjectTemplates in '..\Source\Core\RadIA.Core.DextProjectTemplates.pas',
  RadIA.Core.ProjectTemplates in '..\Source\Core\RadIA.Core.ProjectTemplates.pas',
  RadIA.Core.ProjectTransaction in '..\Source\Core\RadIA.Core.ProjectTransaction.pas',
  RadIA.Core.ProjectTemplateService in '..\Source\Core\RadIA.Core.ProjectTemplateService.pas',
  RadIA.Core.ProjectTemplateTools in '..\Source\Core\RadIA.Core.ProjectTemplateTools.pas',
  RadIA.Core.HttpClient in '..\Source\Core\RadIA.Core.HttpClient.pas',
  RadIA.Core.ErrorDecoder in '..\Source\Core\RadIA.Core.ErrorDecoder.pas',
  RadIA.Core.Localizer in '..\Source\Core\RadIA.Core.Localizer.pas',
  RadIA.OTA.Adapter in '..\Source\Integration\RadIA.OTA.Adapter.pas',
  RadIA.OTA.Workspace in '..\Source\Integration\RadIA.OTA.Workspace.pas',
  RadIA.OTA.Designer in '..\Source\Integration\RadIA.OTA.Designer.pas',
  RadIA.OTA.Debugger in '..\Source\Integration\RadIA.OTA.Debugger.pas',
  RadIA.OTA.RuntimeProcess in '..\Source\Integration\RadIA.OTA.RuntimeProcess.pas',
  RadIA.OTA.RuntimeDiscovery in '..\Source\Integration\RadIA.OTA.RuntimeDiscovery.pas',
  RadIA.OTA.TextReader in '..\Source\Integration\RadIA.OTA.TextReader.pas',
  RadIA.OTA.Knowledge in '..\Source\Integration\RadIA.OTA.Knowledge.pas',
  RadIA.OTA.KnowledgeNotifier in '..\Source\Integration\RadIA.OTA.KnowledgeNotifier.pas',
  RadIA.OTA.InlineReviews in '..\Source\Integration\RadIA.OTA.InlineReviews.pas',
  RadIA.OTA.InlineCompletion in '..\Source\Integration\RadIA.OTA.InlineCompletion.pas',
  RadIA.OTA.IDENavigation in '..\Source\Integration\RadIA.OTA.IDENavigation.pas',
  RadIA.OTA.Consent in '..\Source\Integration\RadIA.OTA.Consent.pas',
  RadIA.OTA.Build in '..\Source\Integration\RadIA.OTA.Build.pas',
  RadIA.MCP.NamedPipe in '..\Source\Integration\RadIA.MCP.NamedPipe.pas',
  RadIA.OTA.Helper in '..\Source\Integration\RadIA.OTA.Helper.pas',
  RadIA.OTA.EditorHook in '..\Source\Integration\RadIA.OTA.EditorHook.pas',
  RadIA.OTA.ContextParser in '..\Source\Integration\RadIA.OTA.ContextParser.pas',
  RadIA.OTA.MessageViewHook in '..\Source\Integration\RadIA.OTA.MessageViewHook.pas',
  RadIA.Core.Mediator in '..\Source\Core\RadIA.Core.Mediator.pas',
  RadIA.Core.OAuth in '..\Source\Core\RadIA.Core.OAuth.pas',
  RadIA.Core.IndyLoopback in '..\Source\Core\RadIA.Core.IndyLoopback.pas',
  RadIA.Provider.Streaming in '..\Source\Providers\RadIA.Provider.Streaming.pas',
  RadIA.Provider.Base in '..\Source\Providers\RadIA.Provider.Base.pas',
  RadIA.Provider.Gemini in '..\Source\Providers\RadIA.Provider.Gemini.pas',
  RadIA.Provider.OpenAI in '..\Source\Providers\RadIA.Provider.OpenAI.pas',
  RadIA.Provider.Claude in '..\Source\Providers\RadIA.Provider.Claude.pas',
  RadIA.Provider.Ollama in '..\Source\Providers\RadIA.Provider.Ollama.pas',
  RadIA.Provider.DeepSeek in '..\Source\Providers\RadIA.Provider.DeepSeek.pas',
  RadIA.Provider.Groq in '..\Source\Providers\RadIA.Provider.Groq.pas',
  RadIA.Provider.OpenRouter in '..\Source\Providers\RadIA.Provider.OpenRouter.pas',
  RadIA.Provider.Generic in '..\Source\Providers\RadIA.Provider.Generic.pas',
  RadIA.Provider.LMStudio in '..\Source\Providers\RadIA.Provider.LMStudio.pas',
  RadIA.Provider.AzureOpenAI in '..\Source\Providers\RadIA.Provider.AzureOpenAI.pas',
  RadIA.Provider.Qwen in '..\Source\Providers\RadIA.Provider.Qwen.pas',
  RadIA.Provider.Mistral in '..\Source\Providers\RadIA.Provider.Mistral.pas',
  RadIA.Core.AwsSigner in '..\Source\Core\RadIA.Core.AwsSigner.pas',
  RadIA.Core.TextNormalizer in '..\Source\Core\RadIA.Core.TextNormalizer.pas',
  RadIA.Provider.GithubCopilot in '..\Source\Providers\RadIA.Provider.GithubCopilot.pas',
  RadIA.Provider.Bedrock in '..\Source\Providers\RadIA.Provider.Bedrock.pas',
  RadIA.UI.ChatPresenter in '..\Source\UI\RadIA.UI.ChatPresenter.pas',
  RadIA.UI.ConfigPresenter in '..\Source\UI\RadIA.UI.ConfigPresenter.pas',

  // Test Suites
  RadIA.Tests.Config in 'Source\RadIA.Tests.Config.pas',
  RadIA.Tests.DelphiEnvironment in 'Source\RadIA.Tests.DelphiEnvironment.pas',
  RadIA.Tests.DelphiGuidance in 'Source\RadIA.Tests.DelphiGuidance.pas',
  RadIA.Tests.DelphiMentor in 'Source\RadIA.Tests.DelphiMentor.pas',
  RadIA.Tests.DfmPasAudit in 'Source\RadIA.Tests.DfmPasAudit.pas',
  RadIA.Tests.DfmPasAuditTools in 'Source\RadIA.Tests.DfmPasAuditTools.pas',
  RadIA.Tests.DesignerVisualDiff in 'Source\RadIA.Tests.DesignerVisualDiff.pas',
  RadIA.Tests.DesignerVisualDiffTools in 'Source\RadIA.Tests.DesignerVisualDiffTools.pas',
  RadIA.Tests.AgentExecutionContract in 'Source\RadIA.Tests.AgentExecutionContract.pas',
  RadIA.Tests.LegacyDataMigration in 'Source\RadIA.Tests.LegacyDataMigration.pas',
  RadIA.Tests.LegacyDataMigrationTools in 'Source\RadIA.Tests.LegacyDataMigrationTools.pas',
  RadIA.Tests.EditorHook in 'Source\RadIA.Tests.EditorHook.pas',
  RadIA.Tests.ContextParser in 'Source\RadIA.Tests.ContextParser.pas',
  RadIA.Tests.Providers in 'Source\RadIA.Tests.Providers.pas',
  RadIA.Tests.Cache in 'Source\RadIA.Tests.Cache.pas',
  RadIA.Tests.Ollama in 'Source\RadIA.Tests.Ollama.pas',
  RadIA.Tests.Service in 'Source\RadIA.Tests.Service.pas',
  RadIA.Tests.PromptHistory in 'Source\RadIA.Tests.PromptHistory.pas',
  RadIA.Tests.TokenUsage in 'Source\RadIA.Tests.TokenUsage.pas',
  RadIA.Tests.Exporter in 'Source\RadIA.Tests.Exporter.pas',
  RadIA.Tests.Templates in 'Source\RadIA.Tests.Templates.pas',
  RadIA.Tests.ProjectContext in 'Source\RadIA.Tests.ProjectContext.pas',
  RadIA.Tests.Streaming in 'Source\RadIA.Tests.Streaming.pas',
  RadIA.Tests.ProvidersEx in 'Source\RadIA.Tests.ProvidersEx.pas',
  RadIA.Tests.Logger in 'Source\RadIA.Tests.Logger.pas',
  RadIA.Tests.Sessions in 'Source\RadIA.Tests.Sessions.pas',
  RadIA.Tests.Quota in 'Source\RadIA.Tests.Quota.pas',
  RadIA.Tests.DTOGenerator in 'Source\RadIA.Tests.DTOGenerator.pas',
  RadIA.Tests.JSONProviders in 'Source\RadIA.Tests.JSONProviders.pas',
  RadIA.Tests.Analysis in 'Source\RadIA.Tests.Analysis.pas',
  RadIA.Tests.ChatPresenter in 'Source\RadIA.Tests.ChatPresenter.pas',
  RadIA.Tests.ConfigPresenter in 'Source\RadIA.Tests.ConfigPresenter.pas',
  RadIA.Tests.ProjectGenerator in 'Source\RadIA.Tests.ProjectGenerator.pas',
  RadIA.Tests.ProjectFiles in 'Source\RadIA.Tests.ProjectFiles.pas',
  RadIA.Tests.ProductivityGeneration in 'Source\RadIA.Tests.ProductivityGeneration.pas',
  RadIA.Tests.StackTraceAnalysis in 'Source\RadIA.Tests.StackTraceAnalysis.pas',
  RadIA.Tests.SaveReview in 'Source\RadIA.Tests.SaveReview.pas',
  RadIA.Tests.CleanUses in 'Source\RadIA.Tests.CleanUses.pas',
  RadIA.Tests.ThreadingAssistant in 'Source\RadIA.Tests.ThreadingAssistant.pas',
  RadIA.Tests.OpenApiRetrofit in 'Source\RadIA.Tests.OpenApiRetrofit.pas',
  RadIA.Tests.DextFormModernization in 'Source\RadIA.Tests.DextFormModernization.pas',
  RadIA.Tests.ApiSpecifications in 'Source\RadIA.Tests.ApiSpecifications.pas',
  RadIA.Tests.DextProjectTemplates in 'Source\RadIA.Tests.DextProjectTemplates.pas',
  RadIA.Tests.ProjectTemplates in 'Source\RadIA.Tests.ProjectTemplates.pas',
  RadIA.Tests.ProjectTemplateTools in 'Source\RadIA.Tests.ProjectTemplateTools.pas',
  RadIA.Tests.Container in 'Source\RadIA.Tests.Container.pas',
  RadIA.Tests.Infrastructure in 'Source\RadIA.Tests.Infrastructure.pas',
  RadIA.Tests.OAuth in 'Source\RadIA.Tests.OAuth.pas',
  RadIA.Tests.IndyLoopback in 'Source\RadIA.Tests.IndyLoopback.pas',
  RadIA.Tests.TextNormalizer in 'Source\RadIA.Tests.TextNormalizer.pas',
  RadIA.Tests.AwsSigner in 'Source\RadIA.Tests.AwsSigner.pas',
  RadIA.Tests.Mediator in 'Source\RadIA.Tests.Mediator.pas',
  RadIA.Tests.ProviderBooster in 'Source\RadIA.Tests.ProviderBooster.pas',
  RadIA.Tests.CoverageBooster in 'Source\RadIA.Tests.CoverageBooster.pas',
  RadIA.Tests.OTAHelper in 'Source\RadIA.Tests.OTAHelper.pas',
  RadIA.Tests.HttpClient in 'Source\RadIA.Tests.HttpClient.pas',
  RadIA.Tests.ToolRegistry in 'Source\RadIA.Tests.ToolRegistry.pas',
  RadIA.Tests.ToolViews in 'Source\RadIA.Tests.ToolViews.pas',
  RadIA.Tests.Problems in 'Source\RadIA.Tests.Problems.pas',
  RadIA.Tests.AgentRuntime in 'Source\RadIA.Tests.AgentRuntime.pas',
  RadIA.Tests.ResultCompactor in 'Source\RadIA.Tests.ResultCompactor.pas',
  RadIA.Tests.ResultCompactionBenchmark in 'Source\RadIA.Tests.ResultCompactionBenchmark.pas',
  RadIA.Tests.ResultCompactionSettings in 'Source\RadIA.Tests.ResultCompactionSettings.pas',
  RadIA.Tests.AgentResultStore in 'Source\RadIA.Tests.AgentResultStore.pas',
  RadIA.Tests.AgentExecutors in 'Source\RadIA.Tests.AgentExecutors.pas',
  RadIA.Tests.CliProcess in 'Source\RadIA.Tests.CliProcess.pas',
  RadIA.Tests.Terminal in 'Source\RadIA.Tests.Terminal.pas',
  RadIA.Tests.TerminalDiagnostics in 'Source\RadIA.Tests.TerminalDiagnostics.pas',
  RadIA.Tests.WebViewLifecycle in 'Source\RadIA.Tests.WebViewLifecycle.pas',
  RadIA.Tests.Onboarding in 'Source\RadIA.Tests.Onboarding.pas',
  RadIA.Tests.CliManager in 'Source\RadIA.Tests.CliManager.pas',
  RadIA.Tests.CliMcpSettings in 'Source\RadIA.Tests.CliMcpSettings.pas',
  RadIA.Tests.McpProvisioning in 'Source\RadIA.Tests.McpProvisioning.pas',
  RadIA.Tests.McpHandshake in 'Source\RadIA.Tests.McpHandshake.pas',
  RadIA.Tests.ExternalMcp in 'Source\RadIA.Tests.ExternalMcp.pas',
  RadIA.Tests.ExternalMcpClient in 'Source\RadIA.Tests.ExternalMcpClient.pas',
  RadIA.Tests.ExternalMcpSecurity in 'Source\RadIA.Tests.ExternalMcpSecurity.pas',
  RadIA.Tests.ExternalMcpSettings in 'Source\RadIA.Tests.ExternalMcpSettings.pas',
  RadIA.Tests.WorkspaceTools in 'Source\RadIA.Tests.WorkspaceTools.pas',
  RadIA.Tests.WorkspaceBoundary in 'Source\RadIA.Tests.WorkspaceBoundary.pas',
  RadIA.Tests.Patches in 'Source\RadIA.Tests.Patches.pas',
  RadIA.Tests.BlockReviews in 'Source\RadIA.Tests.BlockReviews.pas',
  RadIA.Tests.BlockReviewSessions in 'Source\RadIA.Tests.BlockReviewSessions.pas',
  RadIA.Tests.MultiFilePatches in 'Source\RadIA.Tests.MultiFilePatches.pas',
  RadIA.Tests.DevelopmentTransactions in 'Source\RadIA.Tests.DevelopmentTransactions.pas',
  RadIA.Tests.BuildTools in 'Source\RadIA.Tests.BuildTools.pas',
  RadIA.Tests.DUnitX in 'Source\RadIA.Tests.DUnitX.pas',
  RadIA.Tests.Coverage in 'Source\RadIA.Tests.Coverage.pas',
  RadIA.Tests.Git in 'Source\RadIA.Tests.Git.pas',
  RadIA.Tests.Mcp in 'Source\RadIA.Tests.Mcp.pas',
  RadIA.Tests.DesignerTools in 'Source\RadIA.Tests.DesignerTools.pas',
  RadIA.Tests.DesignerMutations in 'Source\RadIA.Tests.DesignerMutations.pas',
  RadIA.Tests.DesignerProperties in 'Source\RadIA.Tests.DesignerProperties.pas',
  RadIA.Tests.DebuggerTools in 'Source\RadIA.Tests.DebuggerTools.pas',
  RadIA.Tests.DebugTimeline in 'Source\RadIA.Tests.DebugTimeline.pas',
  RadIA.Tests.DebuggerControlTools in 'Source\RadIA.Tests.DebuggerControlTools.pas',
  RadIA.Tests.DebuggerBreakpointTools in 'Source\RadIA.Tests.DebuggerBreakpointTools.pas',
  RadIA.Tests.DebuggerInspectionTools in 'Source\RadIA.Tests.DebuggerInspectionTools.pas',
  RadIA.Tests.RuntimeAutomation in 'Source\RadIA.Tests.RuntimeAutomation.pas',
  RadIA.Tests.RuntimeVclAdapter in 'Source\RadIA.Tests.RuntimeVclAdapter.pas',
  RadIA.Tests.RuntimeDebugSession in 'Source\RadIA.Tests.RuntimeDebugSession.pas',
  RadIA.Tests.VisualRuntimeSession in 'Source\RadIA.Tests.VisualRuntimeSession.pas',
  RadIA.Tests.RuntimeVisualTools in 'Source\RadIA.Tests.RuntimeVisualTools.pas',
  RadIA.Tests.RuntimeDebugTools in 'Source\RadIA.Tests.RuntimeDebugTools.pas',
  RadIA.Tests.RuntimeDiscovery in 'Source\RadIA.Tests.RuntimeDiscovery.pas',
  RadIA.Tests.RuntimeScenario in 'Source\RadIA.Tests.RuntimeScenario.pas',
  RadIA.Tests.RuntimeEvidence in 'Source\RadIA.Tests.RuntimeEvidence.pas',
  RadIA.Tests.RuntimeRegression in 'Source\RadIA.Tests.RuntimeRegression.pas',
  RadIA.Tests.MemoryDiagnostics in 'Source\RadIA.Tests.MemoryDiagnostics.pas',
  RadIA.Tests.FastMM5 in 'Source\RadIA.Tests.FastMM5.pas',
  RadIA.Tests.MemoryInstrumentation in 'Source\RadIA.Tests.MemoryInstrumentation.pas',
  RadIA.Tests.FastMM5LogParser in 'Source\RadIA.Tests.FastMM5LogParser.pas',
  RadIA.Tests.MemoryDiagnosticSession in 'Source\RadIA.Tests.MemoryDiagnosticSession.pas',
  RadIA.Tests.MemoryEvidence in 'Source\RadIA.Tests.MemoryEvidence.pas',
  RadIA.Tests.InlineReviews in 'Source\RadIA.Tests.InlineReviews.pas',
  RadIA.Tests.InlineCompletion in 'Source\RadIA.Tests.InlineCompletion.pas',
  RadIA.Tests.IDENavigation in 'Source\RadIA.Tests.IDENavigation.pas',
  RadIA.Tests.Knowledge in 'Source\RadIA.Tests.Knowledge.pas',
  RadIA.Tests.RemoteKnowledgeEmbeddings in 'Source\RadIA.Tests.RemoteKnowledgeEmbeddings.pas',
  RadIA.Tests.KnowledgeEmbeddingSelection in 'Source\RadIA.Tests.KnowledgeEmbeddingSelection.pas',
  RadIA.Tests.RemoteKnowledgeSettings in 'Source\RadIA.Tests.RemoteKnowledgeSettings.pas',
  RadIA.Tests.KnowledgePrivacy in 'Source\RadIA.Tests.KnowledgePrivacy.pas',
  RadIA.Tests.ProjectHealthTools in 'Source\RadIA.Tests.ProjectHealthTools.pas',
  RadIA.Tests.InstallationHealthTools in 'Source\RadIA.Tests.InstallationHealthTools.pas',
  RadIA.Tests.Journeys in 'Source\RadIA.Tests.Journeys.pas',
  RadIA.Tests.IntentRouter in 'Source\RadIA.Tests.IntentRouter.pas',
  RadIA.Tests.IntentTelemetry in 'Source\RadIA.Tests.IntentTelemetry.pas',
  RadIA.Tests.KnowledgeScheduler in 'Source\RadIA.Tests.KnowledgeScheduler.pas',
  RadIA.Tests.KnowledgeNotifier in 'Source\RadIA.Tests.KnowledgeNotifier.pas',
  RadIA.Tests.Extensions in 'Source\RadIA.Tests.Extensions.pas',
  RadIA.Tests.DeclarativeExtensions in 'Source\RadIA.Tests.DeclarativeExtensions.pas',
  RadIA.Tests.DeclarativeExtensionPackages in 'Source\RadIA.Tests.DeclarativeExtensionPackages.pas',
  RadIA.Tests.ExtensionCatalog in 'Source\RadIA.Tests.ExtensionCatalog.pas',
  RadIA.Tests.ExtensionStudio in 'Source\RadIA.Tests.ExtensionStudio.pas',
  RadIA.Tests.SkillPortability in 'Source\RadIA.Tests.SkillPortability.pas',
  RadIA.Tests.SkillReplicas in 'Source\RadIA.Tests.SkillReplicas.pas',
  RadIA.Tests.DesignerComponents in 'Source\RadIA.Tests.DesignerComponents.pas',
  RadIA.Tests.DesignerEvents in 'Source\RadIA.Tests.DesignerEvents.pas',
  RadIA.Tests.ToolSecurity in 'Source\RadIA.Tests.ToolSecurity.pas',
  RadIA.Tests.JourneyContext in 'Source\RadIA.Tests.JourneyContext.pas',
  RadIA.Tests.HierarchicalSettings in 'Source\RadIA.Tests.HierarchicalSettings.pas';

var
  EvidencePath: string;
  Runner: ITestRunner;
  Results: IRunResults;
  Logger: ITestLogger;
  XmlLogger: ITestLogger;
begin
  try
    TRadIAConfig.SetBaseRegistryPath('Software\RadIA\Tests');
    TDUnitX.CheckCommandLine;
    Runner := TDUnitX.CreateRunner;
    Runner.UseRTTI := True;

    Logger := TDUnitXConsoleLogger.Create(True);
    Runner.AddLogger(Logger);

    XmlLogger := TDUnitXXMLNUnitFileLogger.Create(
      TDUnitX.Options.XMLOutputFile
    );
    Runner.AddLogger(XmlLogger);

    Results := Runner.Execute;

    EvidencePath := GetEnvironmentVariable(
      'RADIA_RESULT_COMPACTION_EVIDENCE_PATH'
    );
    if EvidencePath <> '' then
    begin
      TDirectory.CreateDirectory(ExtractFileDir(EvidencePath));
      TFile.WriteAllText(
        EvidencePath,
        BuildRadIAResultCompactionEvidence,
        TEncoding.UTF8
      );
    end;

    if not Results.AllPassed then
      System.ExitCode := 1
    else
      System.ExitCode := 0;
  except
    on E: Exception do
    begin
      Writeln(E.ClassName, ': ', E.Message);
      System.ExitCode := 1;
    end;
  end;
end.
