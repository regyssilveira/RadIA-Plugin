...
procedure TTestRadIAProviders.TestOpenAI_RunCodexLoop_SemanticErrorPath;
var
  LCallback: TCompletionCallback;
  LCallbackCalled: Boolean;
  LCmdLine: string;
...
begin
  ...
  LCallbackCalled := False;
  LCmdLine :=
    'cmd.exe /c echo {"type":"item.completed","item":{' +
    '"text":"Error: No response generated by Codex."}}';
  LCallback :=
    procedure(
      const AResponse: string;
      const AError: string;
