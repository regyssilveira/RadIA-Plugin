...
type
  ...
  public
    ...
    [Test]
    procedure TestExportMarkdown_RedactsSecretsWithDefaultRedactor;
  end;
...
procedure TTestRadIAExporter.TestExportMarkdown_RedactsSecretsWithDefaultRedactor;
var
  LHistory: TArray<IRadIAChatMessage>;
  LMarkdown: string;
begin
  LHistory := TArray<IRadIAChatMessage>.Create(
    TRadIAChatMessage.CreateMessage(mrUser, 'Authorization: Bearer sk-default-secret-123')
  );

  LMarkdown := TConversationExporter.ExportToMarkdown(LHistory, 'OpenAI', 'gpt-5');

  Assert.IsFalse(LMarkdown.Contains('sk-default-secret-123'));
  Assert.IsTrue(LMarkdown.Contains('Bearer [REDACTED]'));
end;
