...
* `docs/project/backlog.md` and `docs/project/backlog.en.md`: open work only.
* `docs/project/roadmap.md` and `docs/project/roadmap.en.md`: future direction without release history.
* GitHub Releases: version-specific notes, artifacts, and evidence.
...
The script also overrides the Delphi installation and compiler used by the analyzer according to
`-DelphiVersion`; this prevents an old server setting from analyzing the release as Delphi 11.
Use Delphi 12 (`VER360`) for Sonar until the community analyzer fully parses the Delphi 13 RTL.
Builds and tests remain mandatory on both versions and on IDE64.
...
`radia-delphi` label, Delphi 12, and `sonar-scanner`; configure `SONAR_TOKEN` as a repository secret
...
Visual acceptance opens the unit through the native IDE dialog, waits for and activates
`TEditControl`, requests its repaint, and passes only when the real OTA callback confirms that
completion and reviews reached the paint cycle. The smoke does not bypass consent for protected
RadIA navigation.
...
The gate creates `terminal_high_fidelity_evidence_2.6.0.json`; it complements but does not replace
...
The current result is stored in `terminal_smoke_evidence_2.6.0.json`. The 2.0.0 file remains the
...
The gate requires exactly three supported targets, 148 tools, and the
...
The gate requires exactly Delphi 12 Win32, Delphi 13 Win32, and Delphi 13 IDE64, all with 148 tools,
...
Publish only the installer. Version 2.8.0 has no automatic updater and does not publish
`stable.json`. See [Visual installer](../getting-started/visual_installer.en.md) for the complete flow.
...
* Visual installer and SHA-256 published.
