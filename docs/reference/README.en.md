# Reference

- [Everything RadIA can do](capabilities.en.md): canonical user-oriented reference
- [Detailed feature inventory](features.en.md): individual features, categories, and status
- [Settings](settings_reference.en.md)
- [Slash commands](slash_commands.en.md)
- [Doctor](doctor.en.md)
- [Internal tools](internal_tools_reference.en.md)
- [Runtime catalog](runtime_tool_catalog.en.md)
- [Runtime catalog JSON](runtime_tools.json)
- [CLI capability matrix](cli_capability_matrix.en.md)
- [Delphi environment profile](delphi_environment_profile.en.md)
- [Delphi compatibility](delphi_compatibility_matrix.en.md)
- [Delphi guidance](delphi_guidance.en.md)
- [Security model](tool_security_model.en.md)
- [Corporate security brief](corporate_security_brief.en.md)
- [Agent pricing](agent_pricing.en.md)
- [Extension catalog](extension_catalog.en.md)
