...
|`FindSymbolReferences`|Finds confirmed Pascal and DFM declarations and references with file, line, and column.|When the user asks for usages or references; homonyms require a unit and ambiguous occurrences appear only when explicitly requested as candidates.|
|`GetTypeHierarchy`|Returns ancestors and descendants of an indexed Delphi type, including depth and external-type markers.|When the user or agent must understand inheritance before navigation, refactoring, or impact analysis; homonyms require a unit.|
|`PrepareRenameSymbol`|Prepares an exact semantic rename across Pascal and DFM files, including closed files and optional coordination of members across ancestors and overrides.|When the user requests a symbol rename; for hierarchies use `container`, `unit`, `includeHierarchy`, and `signature` for overloads. It always produces a preview for multi-file application and rollback.|
...
| `GetFireDACProjectReport` | Aggregates inventory and sanitized analysis of embedded FireDAC SQL. | When reviewing the FireDAC layer with navigable locations. |
...
| `InspectFireDACConfiguration` | Inspects bounded FireDAC configuration while discarding credentials and absolute paths. | When reviewing drivers, connection definitions, options, and libraries before execution. |
...
| `GenerateFireDACSchemaReport` | Generates a sanitized FireDAC report from the local SQLite schema without querying rows. | When documenting tables, views, columns, and sensitive flags in the authorized database. |
