...
disponíveis pela API BPL descrita no [guia de extensões](../development/tool_extension_guide.md).
...
A matriz funcional é validada pelos testes automatizados e pelas evidências publicadas no GitHub
Releases. Ela comprova hot-load, registro no catálogo compartilhado e execução auditada de duas etapas no
...
de pacotes estão descritos em [Catálogo remoto de extensões](../reference/extension_catalog.md).
