# Guias de uso

Guias orientados a tarefas. Para campos, comandos e catálogos, consulte as
[referências](../reference/README.md).

## Chat, agente e contexto

- [Manual do usuário](user_manual.md)
- [Chat e sessões](user_guide_chat_sessions.md)
- [Ferramentas agentivas](user_guide_agentic_tools.md)
- [Contrato de execução autônoma](autonomous_execution_contract.md)
- [Contexto compartilhado da jornada](shared_journey_context.md)
- [Compactação de resultados](agent_result_compaction.md)
- [Configurações hierárquicas](hierarchical_settings.md)

## Código e projetos

- [Editor e geração](user_guide_editor_generation.md)
- [Jornadas ponta a ponta](user_guide_journeys.md)
- [New Project Wizard](project_wizard.md)
- [Operações estruturais de projeto](project_file_operations.md)
- [Documentação de API e mocks seguros](safe_productivity_tools.md)
- [Gerenciamento do cache](cache_management.md)
- [Transações de desenvolvimento](development_transactions.md)
- [Conhecimento do projeto](user_guide_project_knowledge.md)
- [Mentor Delphi](delphi_mentor.md)
- [Runner DUnitX](dunitx_runner.md)
- [Fluxo Git](git_workflow.md)
- [Migração de acesso a dados legado](legacy_data_migration.md)
- [Jornadas DEXT](user_guide_dext_journeys.md)

## Editor, Designer e revisão

- [Assistência inline e FIM](inline_completion.md)
- [Revisão por bloco](block_reviews.md)
- [Designer e debugger](user_guide_designer_debugger.md)
- [Auditoria DFM/PAS](dfm_pas_audit.md)
- [Diff visual do Designer](designer_visual_diff.md)

## Diagnóstico

- [Diagnóstico e análise](user_guide_diagnostics_analysis.md)
- [Automação de diagnóstico runtime](runtime_debug_automation.md)
- [Configuração do FastMM5](fastmm5_configuration.md)
- [Instrumentação FastMM5](fastmm5_instrumentation.md)
- [Sessão de diagnóstico de memória](fastmm5_diagnostic_session.md)
- [Parser de evidências FastMM5](fastmm5_evidence_parser.md)
- [Solução de problemas](troubleshooting_agentic_platform.md)

## Integrações

- [Executores nativo e CLI](cli_executors.md)
- [Terminal](terminal.md)
- [Integração MCP](mcp_integration_guide.md)
- [Proxy do GitHub Copilot](copilot_proxy_guide.md)
- [Extensões declarativas](declarative_extensions.md)
- [Portabilidade de skills](skill_portability.md)
