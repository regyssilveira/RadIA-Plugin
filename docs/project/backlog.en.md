...
Fifteen scenarios are connected and passing on all three targets, producing 45 real executions:
...
legacy source, and project switching/reopening without stale context. The shutdown scenario remains
to be connected, and the smoke's controlled SQL input must be replaced with a real editor selection.
