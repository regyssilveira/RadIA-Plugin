...
Ten scenarios are connected and passing on all three targets, producing 30 real executions:
...
after a failed build gate. The other six scenarios remain to be connected, and the smoke's
controlled SQL input must be replaced with a real editor selection.
