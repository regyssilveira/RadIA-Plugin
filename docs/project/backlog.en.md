...
Eleven scenarios are connected and passing on all three targets, producing 33 real executions:
...
after a failed build gate, plus the consented parameter accessor fix with Smart Diff, build, and
DUnitX. The other five scenarios remain to be connected, and the smoke's controlled SQL input must
be replaced with a real editor selection.
