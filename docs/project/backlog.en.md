...
Twelve scenarios are connected and passing on all three targets, producing 36 real executions:
...
and stale-preview rejection without overwriting a later change. The other four scenarios remain to
be connected, and the smoke's controlled SQL input must be replaced with a real editor selection.
