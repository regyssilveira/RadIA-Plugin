...
Thirteen scenarios are connected and passing on all three targets, producing 39 real executions:
...
inspection, build, DUnitX, and a validated gate. The other three scenarios remain to be connected,
and the smoke's controlled SQL input must be replaced with a real editor selection.
