...
conexão completa desse plano ao runner MCP/IDE real e a coleta das evidências runtime. Nove cenários
já estão conectados e aprovados nos três targets, totalizando 27 execuções reais: inventário,
...
com aplicação negada ou aplicação consentida seguida por build e DUnitX. Ainda faltam conectar os
outros sete cenários e substituir a entrada SQL controlada do smoke pela captura real da seleção no
editor.
